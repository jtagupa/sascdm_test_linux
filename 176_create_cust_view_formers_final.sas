...
******************************************************************************;
/*filename myfile ftp "'OF.WSJ.PWE.DAILYMF(0)'"
          host='162.55.33.54' prompt
	         	recfm=f rcmd='site rdw' lrecl=1458;  */
...
data price_&date_mmddyy.; set mypwolf3(keep=Prod_Code Acct_No Acct_No_hash Company Prefix	Firstname Lastname	Suffix Street_number 
...
data price_&date_mmddyy.; set price_&date_mmddyy.;
	...
	/*
	IF '000'<=CURRENT_TERM<= '035' THEN Term_1='5'; 
	IF '036'<=CURRENT_TERM<= '070' THEN Term_1='8'; 
	IF '071'<=CURRENT_TERM<= '098' THEN Term_1='13'; 
	IF '099'<=CURRENT_TERM<= '140' THEN Term_1='17'; 
	IF '141'<=CURRENT_TERM<= '182' THEN Term_1='26';
	IF '183'<=CURRENT_TERM<= '315' THEN Term_1='30';
	IF '316'<=CURRENT_TERM<= '537' THEN Term_1='52';
	IF '538'<=CURRENT_TERM THEN Term_1='104';   */
...
length date $8.;
length _calcdate $4.;
_yr=substr(Acct_No_hash,5,2);
_calcdate=substr(Acct_No_hash,1,4);
...
if prod_code = ('J') then do;
if term_commmit>0 and commit_date ^ = 99999999  then do;
	if cust_status="Live" and todaydate >= commit_date then Cust_Status3='Grace'; 
...
if cust_status="Live" and date2 >= expire_date then Cust_Status3='Grace'; 
else Cust_Status3=cust_status;
...
data myemail;
   	set stg.LOADFILE;
	label	Acct_No_hash="Customer Unique Account Number"
            Prod_Code="Product Code"
			Email_Address_hash="Subscribers Email Address"
			Renew_Via_Email="Renewal Subscription via email"
			Req_Ques_Via_Email="Receive Questioniare via email"
			Req_Djps_Via_Email="Receive Solicitations via email"  
			Wsjie_Sub="Subscription to WSJIE Indicator";
...
data &dsnout ;
	...
	(email_address_hash ^ ? 'spam' & email_address_hash ^ ? 'abuse' &
	email_address_hash ^ ? 'dowjones' & email_address_hash ^ ? 'DOWJONES' &
	email_address_hash ^ ? 'wsj' & email_address_hash ^ ? 'WSJ' &
	email_address_hash ^ ? 'fmr' & email_address_hash ^ ? 'FMR' &
	email_address_hash ^ ? 'factiva' & email_address_hash ^ ? 'FACTIVA' &
	email_address_hash ^ ? 'SPAM' & email_address_hash ^ ? 'ABUSE')) ) ;
	keep ACCT_NO Acct_No_hash email_address
	 Email_Address_hash Req_Djps_Via_Email Req_Ques_Via_Email;
run;

/**data work.wsjemail; set work.wsjemail; email_address=left(email_address); run;
data work.wsjemail; set work.wsjemail; email_address=lowcase(email_address); run; */
proc sort nodupkey data=work.wsjemail; by email_address_hash; run;
data work.wsjemail; set work.wsjemail; where email_address_hash not in (' ',"&hash_null"); run;


data work.wsjemail; set work.wsjemail; where acct_no_hash ^="000000000000"; run;


PROC SORT NODUPKEY DATA=work.wsjemail; BY acct_no_hash; RUN;
PROC SORT NODUPKEY DATA=work.price_&date_mmddyy._final; BY acct_no_hash; RUN;

data work.price_&date_mmddyy._final_email;
merge work.price_&date_mmddyy._final(in=in1) work.wsjemail(in=in2);
by acct_no_hash;
if in1;
run;
...
***LIBNAME cir ODBC DSN='EDW Prod' USER='mktuser' PASSWORD='mktU53r' SCHEMA=CIRCULATION;
...
%mend skip2x;
	...
	run;
		...
		proc sort data=stg.mosaic_cust_subscription(keep=PRINT_ACCT_NUM_hash PRINT_ACCT_NUM   uu_id_hash uu_id 
			...
			out=sorted_mcs;
				by PRINT_ACCT_NUM&hash;
				where PRINT_ACCT_NUM&hash ne '' and PRINT_ACCT_NUM&hash ne "&hash_null";
		run;
...
merge work.price_&date_mmddyy._final_email (in=in1) work.price_&date_mmddyy._final_acct (in=in2 keep = acct_no acct_no_hash);
by acct_no_hash;
if in1 and not in2;
run;
...
proc sort data=stg.mosaic_cust_subscription (keep= Email_Addr_hash email_addr uu_id_hash  uu_id PROD_CD)
		out=mcs_email;
			by EMAIL_ADDR&hash;
	...
	data OLFtomcs_email2;
		...
		if a and b;
			...
			email_hash=left(EMAIL_ADDR_hash);
			email=email_addr;
		**	email =left(EMAIL_ADDR );
			*email=lowcase(EMAIL_ADDR_hash);
	run;
data work.price_&date_mmddyy._final_yesemail; set work.price_&date_mmddyy._final_yesemail; 
length email_hash $200.;
length email $200.;
***email=left(email_address);
email_hash=email_address_hash;
email=email_address;
run;
data work.price_&date_mmddyy._final_acct; set work.price_&date_mmddyy._final_acct; 
length email_hash $200.;
length email $200.;
***email=left(email_address);
email_hash=email_address_hash;
...
merge work.price_&date_mmddyy._final_yesemail (in=in1) OLFtomcs_email2 (in=in2 keep = email_hash email  uu_id uu_id_hash);
by email_hash;
if in1 and in2;
run;
...
merge work.price_&date_mmddyy._final_yesemail (in=in1) work.price_&date_mmddyy._final_yesemail2 (in=in2 keep = acct_no_hash);
by acct_no_hash;
if in1 and not in2;
run;
/*ALL OLF FILES*/
...
data fmr.price_&date_mmddyy._final_ALLOLF;
set price_&date_mmddyy._final_ALLOLF;
run;
...
/*PREPARE ACTIVE FILE*/
 ...
 data work.price_&date_mmddyy._final_ALLOLFFM2; set work.price_&date_mmddyy._final_ALLOLF;
 where cust_status3  = "Stopped";
 run;


/*PROGRAM CODES*/
data olf_trial&date_mmddyy.; set work.price_&date_mmddyy._final_ALLOLFFM2; 
run;
data work.prg_cd4; set slow_chg.prg_cd4; run;
proc sort nodupkey data= work.prg_cd4; by curr_source_key; run;
 proc sort data= olf_trial&date_mmddyy.; by curr_source_key; run;

 data olf_trial_ck;
 merge olf_trial&date_mmddyy.(in=in1) work.prg_cd4 (in=in2 keep= curr_source_key marketing_program campaign_name sk_Start_Date);
 by curr_source_key;
 if in1;
 run;
 ...
 where marketing_program = " ";
 run;
 proc sort nodupkey data= work.prg_cd4; by orig_source; run;
 proc sort data= olf_trial_ok; by orig_source; run;

 data olf_trial_ok;
 merge olf_trial_ok(in=in1) work.prg_cd4 (in=in2 keep= orig_source marketing_program campaign_name sk_Start_Date);
 by orig_source;
 if in1;
 run;
 data work.price_&date_mmddyy._final_ALLOLFFM2(drop = campaign_name sk_Start_Date);
set olf_trial_ck1 olf_trial_ok; run;


data work.price_&date_mmddyy._final_ALLOLFFM2_reg; set work.price_&date_mmddyy._final_ALLOLFFM2(keep= acct_no acct_no_hash  uu_id uu_id_hash); run;


proc sql;

create table work.Price_&date_mmddyy._final_opfm
as
select m.*, d.register_dt
from
   work.price_&date_mmddyy._final_allolffm2_reg m left outer join
   stg.PROV_REG_DATA d
on m.uu_id_hash = d.UU_ID_hash   ;


quit;

data work.Price_&date_mmddyy._final_opfm;
   set work.Price_&date_mmddyy._final_opfm;
...
proc sort nodupkey data= work.price_&date_mmddyy._final_ALLOLFFM2; by acct_no_hash; run;
 proc sort nodupkey data= work.price_&date_mmddyy._final_opfm; by acct_no_hash; run;


 data work.price_&date_mmddyy._final_ALLOLFFM2;
 merge work.price_&date_mmddyy._final_ALLOLFFM2(in=in1) work.price_&date_mmddyy._final_opfm (in=in2 drop = uu_id uu_id_hash);
 by acct_no_hash;
 if in1;
 run;

 
 data work.price_&date_mmddyy._final_ALLOLFFM2;  set work.price_&date_mmddyy._final_ALLOLFFM2;
 registration = prov_reg;
 run;
/*
 proc sql;
create table test_89 as
select uu_id_reg,uu_id_hash,registration,prov_reg,WSJPLUSsub_Type,Cust_Status3, count (distinct Acct_no_hash) as Num_accts
from work.price_092716_final_ALLOLF2

group by uu_id_reg,uu_id_hash,registration,prov_reg,WSJPLUSsub_Type,Cust_Status3;
quit;  */

/*ALL MOSAIC*/
/*BUNDLEPRINT ONLY SUBS*/

data mcs_stp_bun; set stg.mosaic_cust_subscription;
where PROD_CD in ("prod830009") and BUSINESS_OWNER= "CONSUMER US" and
BILL_SYS_ID in (1,8) and SBSCR_TRMNT_DT is not missing;
run;

***LIBNAME cir ODBC DSN='EDW Prod' USER='mktuser' PASSWORD='mktU53r' SCHEMA=CIRCULATION;


data mig; set mcs_stp_bun;
...
date2=input(put(todaydate,8.),yymmdd8.);
format date2 date9.;
numdays=intck('day',INIT_ORD_CRE_DT,SBSCR_TRMNT_DT);
numyrs=intck('year',INIT_ORD_CRE_DT,SBSCR_TRMNT_DT);
nummth=(intck('month',INIT_ORD_CRE_DT,SBSCR_TRMNT_DT)/12);
nummthb=(intck('month',INIT_ORD_CRE_DT,SBSCR_TRMNT_DT));

_stpnummth=(intck('month',SBSCR_TRMNT_DT,date2)/12);
_stpnummthb=(intck('month',SBSCR_TRMNT_DT,date2));
run;

proc freq data = mig; tables _stpnummthb; run;
...
IS_MIGRATED	EXTL_SYS_ID	ICS_CUST_ID	 print_Acct_num	PRINT_ACCT_NUM_HASH	COUPON_CD	INIT_ORD_SOURCE_CD	
...
cur_Src_grp2	subscription_Type	term_wk	num_copies	todaydate	nummth	_stpnummthb email email_hash	curr_Source_key	new_print_stat2
...
data mcs_stp_onl; set stg.mosaic_cust_subscription;
where PROD_CD in ("prod10004") and BILL_SYS_ID in (1,8) and SBSCR_TRMNT_DT is not missing;
run;
/*proc freq data = online2; tables channel; run;  */
/*NATIVE BRM*/
data online2; set mcs_stp_onl;
...
tdat2_yr=year(tdat2);
run;
**proc freq data = online2b; **tables channel; **run;
data online2b; set online2b;
length AR_flag $8.;
length 	new_print_stat $25.;
length Cust_Status3 $25.;
...
run;
/**proc freq data = online2bc; tables channel; run;  **/
...
data work.Price_&date_mmddyy._final_onlineFM; 
length acct_no_hash $200.;
length acct_no $200.;
length new_print_stat2 $25.;
set online2a; 
...
run;
data work.Price_&date_mmddyy._final_onlineFM; set work.Price_&date_mmddyy._final_onlineFM; where SBSCR_TRMNT_DT is not missing; run;

/**** 04/20/2017 save copy in archived folder in case need to re-create the final dataset in future***/

data fmr.Price_&date_mmddyy._final_onlineFM;
set work.Price_&date_mmddyy._final_onlineFM;
run;



data work.Price_&date_mmddyy._final_onlineFM; set work.Price_&date_mmddyy._final_onlineFM(keep=
...
data work.price_&date_mmddyy._final_mcsFM; set allmcs2; 
if DUR_QTY = 2 and DUR_UNIT = "Month" then do;
        TERM_WKS = 8 ;
		Frequency_Desc ="Bi-Monthly";
end;
...
data work.price_&date_mmddyy._final_mcsFM2; set work.price_&date_mmddyy._final_mcsFM;
Length FILENAME2 $25.;
...
data fmr.price_&date_mmddyy._final_mcsFM2;
set work.price_&date_mmddyy._final_mcsFM2;
run;


data work.price_&date_mmddyy._final_mcsFM2; 
set work.price_&date_mmddyy._final_mcsFM2( keep = acct_no ACCT_NO_hash UU_ID UU_ID_hash SBSCR_ID email EMAIL_hash PHONE_NUM CUST_STATUS3 SUBSCRIPTION_TYPE SUB_START_DATE EDW_OFFER_PRICE FREQUENCY_DESC PAYMENT_TYPE MARKETING_PROGRAM EXPIRE_DATE RENEWAL_DATE STOP_DATE NUMMTH _stpnummthb new_print_stat2 cur_Src_grp3 AR_flag SBSCR_STATE_CODE FILENAME2 SOURCE OFFER_TYPE);
run; 


proc sort nodupkey data = work.Price_&date_mmddyy._final; by acct_no_hash; run;
proc sort data = work.price_&date_mmddyy._final_mcsFM2; by acct_no_hash; run;

data work.price_&date_mmddyy._final_mcsFM2; 
merge work.price_&date_mmddyy._final_mcsFM2 (in=in1)  work.Price_&date_mmddyy._final (in =in2 keep= Acct_no Acct_No_hash Company Prefix Firstname Lastname Suffix Street_number Street_name	Extra_address City State Zipcode Zipplus4 Zipplus3 Carrier);
by acct_no_hash;
if in1;
run;

data work.price_&date_mmddyy._final_mcsFM2; set work.price_&date_mmddyy._final_mcsFM2; 
Length FILENAME2 $25.;
Length SOURCE $25.;
FILENAME2 = "Print/Bundle";
SOURCE = "MOSAIC";
...
data work.Price_&date_mmddyy._final_onlineFM2; set work.Price_&date_mmddyy._final_onlineFM;
Length FILENAME2 $25.;
FILENAME2 = "Digital";
Length SOURCE $25.;
SOURCE = "MOSAIC";
...
data work.Price_&date_mmddyy._final_onlineFM2; 
set work.Price_&date_mmddyy._final_onlineFM2( keep =  acct_no ACCT_NO_hash FIRST_NAME LAST_NAME  UU_ID UU_ID_hash SBSCR_ID email EMAIL_hash PHONE_NUM CUST_STATUS3 SUBSCRIPTION_TYPE SUB_START_DATE EDW_OFFER_PRICE FREQUENCY_DESC PAYMENT_TYPE MARKETING_PROGRAM EXPIRE_DATE RENEWAL_DATE STOP_DATE NUMMTH _stpnummthb new_print_stat2 cur_Src_grp3 AR_flag SBSCR_STATE_CODE FILENAME2 SOURCE OFFER_TYPE);
run; 
data work.Price_&date_mmddyy._final_onlineFM2; set work.Price_&date_mmddyy._final_onlineFM2;
...
data work.Price_&date_mmddyy._final_onlinefm2; set work.Price_&date_mmddyy._final_onlineFM2 (keep = acct_no ACCT_NO_hash UU_ID UU_ID_hash SBSCR_ID email  EMAIL_hash); run;

libname RAJ 'Q:\MEIXLER\RAJITHA';
LIBNAME circ ODBC DSN='EDW Prod' USER='mktuser' PASSWORD='mktU53r' SCHEMA=CIRCULATION;*/
...
from
    work.Price_&date_mmddyy._final_onlinefm2 m left outer join
...
create table Price_&date_mmddyy._final_onlinefm2_op
as
...
proc sort data=Price_&date_mmddyy._final_onlinefm2_op nodupkey;
  by sbscr_id;
run;

data work.Price_&date_mmddyy._final_onlinefm2_op;
   set Price_&date_mmddyy._final_onlinefm2_op;
...
data work.Price_&date_mmddyy._final_onlinefm2_opb; set work.Price_&date_mmddyy._final_onlinefm2_op;

length Street_number $200.;
Street_number=street_num;
length Street_name	$200.;
Street_name = addr1;
length Extra_address $200.;
Extra_address = addr2;
length City $200.;
City = City2;
length State $200.;
...
Zipcode = zip_5;
	run;
data work.Price_&date_mmddyy._final_onlinefm2_opb; set work.Price_&date_mmddyy._final_onlinefm2_opb;
/*Firstname=upcase(Firstname);
Lastname=upcase(Lastname);*/
Street_number=upcase(Street_number);
Extra_address=upcase(Extra_address);
City=upcase(City);
State=upcase(State);
run;

proc sort nodupkey data = work.Price_&date_mmddyy._final_onlinefm2_opb; by SBSCR_ID; run;
proc sort data = work.Price_&date_mmddyy._final_onlineFM2; by SBSCR_ID; run;

data work.Price_&date_mmddyy._final_onlineFM2b; 
merge work.Price_&date_mmddyy._final_onlineFM2 (in=in1)  work.Price_&date_mmddyy._final_onlinefm2_opb (in =in2 keep= SBSCR_ID Street_number Street_name Extra_address City State Zipcode);
by SBSCR_ID;
if in1;
run;

data work.price_&date_mmddyy._final_ALLMCS_F; set work.Price_&date_mmddyy._final_onlineFM2b work.Price_&date_mmddyy._final_mcsFM2; run;


/*pull out outlier records*/
data work.price_&date_mmddyy._final_ALLMCS_F; set work.price_&date_mmddyy._final_ALLMCS_F; where substr (SBSCR_STATE_CODE,1,9)  = "CANCELLED" and
subscription_type  ^= "Corporate" and SBSCR_ID > 0 /*and  _stpnummthb < 8*/; run;


data work.price_&date_mmddyy._final_ALLMCS_F; set work.price_&date_mmddyy._final_ALLMCS_F; 
...
data work.price_&date_mmddyy._final_ALLMCS_F2; 
set work.price_&date_mmddyy._final_ALLMCS_F(keep = ACCT_NO ACCT_NO_hash  UU_ID UU_ID_hash SBSCR_ID email EMAIL_hash PHONE_NUM CUST_STATUS3 SUBSCRIPTION_TYPE SUB_START_DATE EDW_OFFER_PRICE FREQUENCY_DESC PAYMENT_TYPE 
MARKETING_PROGRAM EXPIRE_DATE RENEWAL_DATE STOP_DATE NUMMTH new_print_stat2 cur_Src_grp3 AR_flag SBSCR_STATE_CODE registration FILENAME2 SOURCE OFFER_TYPE _stpnummthb WSJPLUSsub_Type Firstname Lastname Street_number Street_name Extra_address City State Zipcode) ; 
where WSJPLUSsub_Type ="Regular";
run;
data work.price_&date_mmddyy._final_ALLMCS_F2; set work.price_&date_mmddyy._final_ALLMCS_F2;
 length _SBSCR_ID $120.;
 _SBSCR_ID = SBSCR_ID; 
Cumm_sub_amt = EDW_OFFER_PRICE;
run;


 data work.price_&date_mmddyy._final_ALLOLFFM2a( keep =ACCT_NO ACCT_NO_hash UU_ID UU_ID_hash email EMAIL_hash PHONE_NUM CUST_STATUS3 SUBSCRIPTION_TYPE SUB_START_DATE CUMM_SUB_AMT FREQUENCY_DESC PAYMENT_TYPE stop_reason
MARKETING_PROGRAM EXPIRE_DATE RENEWAL_DATE STOP_DATE NUMMTH new_print_stat2 cur_Src_grp3 AR_flag registration rate3 FILENAME2 SOURCE _stpnummthb WSJPLUSsub_Type Firstname Lastname Street_number Street_name Extra_address City State Zipcode);  
set work.price_&date_mmddyy._final_ALLOLFFM2;
LENGTH FILENAME2 $25.;
...
SUB_START_DATE_f = mdy(substr(SUB_START_DATE,5,2),substr(SUB_START_DATE,7,2),substr(SUB_START_DATE,1,4));

format SUB_START_DATE_f date9.; 
drop SUB_START_DATE;
 rename SUB_START_DATE_f =SUB_START_DATE;
/*
EXPIRE_DATE=input(put(EXPIRE_DATE,8.),yymmdd8.);
format EXPIRE_DATE date9.; 
*/
RENEWAL_DATE_c=put(RENEWAL_DATE,8.);
RENEWAL_DATE=mdy( substr(RENEWAL_DATE_c,5,2),substr( RENEWAL_DATE_c,7,2),substr( RENEWAL_DATE_c,1,4)); 
format RENEWAL_DATE date9.;

STOP_DATE_c=put(STOP_DATE,8.);
STOP_DATE=mdy( substr(STOP_DATE_c,5,2),substr( STOP_DATE_c,7,2),substr( STOP_DATE_c,1,4)); 
...
/*_stpnummth=(intck('month',STOP_DATE,date2)/12);*/
_stpnummthb=(intck('month',STOP_DATE,date2));
run;


 data work.price_&date_mmddyy._final_ALLOLFFM2a; set work.price_&date_mmddyy._final_ALLOLFFM2a (rename=(rate3=price));
 length SBSCR_STATE_CODE  $40.;
 length _SBSCR_ID $200.;
 _SBSCR_ID = acct_no_hash; 
 if (substr(stop_reason,1,1) in ("2","X")) then SBSCR_STATE_CODE ="CANCELLED_EXPIRED";
 else if (substr(stop_reason,1,1) in ("1")) then SBSCR_STATE_CODE ="CANCELLED_HARD_DECLINED";
 else SBSCR_STATE_CODE ="CANCELLED_BY_CSR";
 run;


  data work.price_&date_mmddyy._final_ALLMCS_F2; set work.price_&date_mmddyy._final_ALLMCS_F2(rename=(EDW_OFFER_PRICE=price));
 length _SBSCR_ID $200.;
 _SBSCR_ID = SBSCR_ID; 
 run;


  data work.price_&date_mmddyy._final_ALLOLFFM2a; set work.price_&date_mmddyy._final_ALLOLFFM2a(drop=/*cumm_sub_amt*/ Stop_Reason) ; run;

  data work.price_&date_mmddyy._final_ALLOLFFM2a; set work.price_&date_mmddyy._final_ALLOLFFM2a;
...
run;
 ...
 data fmr.WSJIP_FORMER_&date_mmddyy.(drop = SBSCR_ID); set work.price_&date_mmddyy._final_ALLOLFFM2a work.price_&date_mmddyy._final_ALLMCS_F2; run;

 data fmr.WSJIP_FORMER_&date_mmddyy._Custview; set fmr.WSJIP_FORMER_&date_mmddyy.;

_CUR_TENURE_yrs=(intck('month',SUB_START_DATE,STOP_DATE)/12);
_CUR_TENURE_mths=(intck('month',SUB_START_DATE,STOP_DATE));
run;
 

 data fmr.WSJIP_FORMER_&date_mmddyy._Custview_ind; set fmr.WSJIP_FORMER_&date_mmddyy._Custview;
where _stpnummthb < 8;
run;
 data fmr.WSJIP_FORMER_&date_mmddyy._Custview_reg; set fmr.WSJIP_FORMER_&date_mmddyy._Custview;
where _stpnummthb < 8 and subscription_type = "Regular";
run;

data fmr.WSJIP_FORMER_&date_mmddyy._Custview; set fmr.WSJIP_FORMER_&date_mmddyy._Custview; where price  ^ = 0; run;
data fmr.WSJIP_FORMER_&date_mmddyy._Custview; set fmr.WSJIP_FORMER_&date_mmddyy._Custview; where price  is not missing; run;

/* RENAME CUR_SRC_GRP3 to Channel, bring offer type, prod_code later add WSJ+ flag, EMEA, APAC, INAPP*/
data fmr.WSJ_FMR_&date_mmddyy._Custview_IND; set  fmr.WSJIP_FORMER_&date_mmddyy._Custview_ind
(drop= 
...
PHONE_NUM
_stpnummthb
/*date3 - not found on dataset***/
);
run;

/**** change dataset name to < 32 char long and drop list*****/
data fmr.WSJIP_FMR_&date_mmddyy._Custview_SFMC; set fmr.WSJIP_FORMER_&date_mmddyy._Custview (drop= 
/*date3 */
PRICE
/*_ORI_TENURE_ys */
WSJPLUSsub_Type
...
***** changed drop list ****;
data fmr.WSJ_FMR_&date_mmddyy._Custview_BLUEK; set fmr.WSJIP_FORMER_&date_mmddyy._Custview_reg
(drop= 
/*date3 */
PRICE
/*_ORI_TENURE_ys */
...
/**** keep only 5 datasets and delete intrim datasets*****/

proc datasets lib=fmr;
      save price_&date_mmddyy._final_mcsFM2 
         Price_&date_mmddyy._final_onlineFM 
         price_&date_mmddyy._final_ALLOLF 
         WSJIP_FORMER_&date_mmddyy. 
		 WSJIP_FORMER_&date_mmddyy._Custview_ind
	     WSJIP_FORMER_&date_mmddyy._CUSTVIEW / memtype=data;
 run;
...
from work.WSJIP_FORMER_&date_mmddyy._Custview_ind;;

quit;

proc export data=indicative_file2 
            outfile="&root./&user./&env./&project./raw-data/indicative_formers_file_&today_yyyymmdd..csv"
			dbms=csv replace;
run;
	...
	filename mymail email 
      ...
      from = "SASCDM &env_email. Alerts <noreply@dowjones.com>"
		...
		data _null_;
			...
			message = "INDICATIVE Formers file for asofdate &today_MMDDYY10. has been created";
      message1 = "&root./&user./&env./&project./raw-data/indicative_formers_file_&today_yyyymmdd..csv has been created." ;
			message2 = " ";
